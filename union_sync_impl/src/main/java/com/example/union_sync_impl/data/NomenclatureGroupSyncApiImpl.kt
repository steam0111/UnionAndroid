...
class NomenclatureGroupSyncApiImpl(
    ...
    override suspend fun getNomenclatureGroups(): List<NomenclatureGroupSyncEntity> {
        ...
        if (nomenclatureDb.isEmpty()) {
            val nomenclaturesNetwork = nomenclatureGroupApi.apiCatalogsNomenclatureGroupGet().list ?: return emptyList()
            nomenclatureGroupDao.insertAll(nomenclaturesNetwork.map { it.toNomenclatureDb() })
