...
class EmployeeSyncApiImpl(
    ...
    private val employeeWorkPlacesDao: EmployeeWorkPlaceDao
) : EmployeeSyncApi {
    ...
    override suspend fun getEmployeeDetail(id: String): EmployeeDetailSyncEntity? {
        ...
        val workPlaces = employeeWorkPlacesDao.getByEmployeeId(id).ifEmpty { null }

        return fullEmployee?.toDetailSyncEntity(
            balanceUnits = balanceUnits,
            structurals = structurals,
            employeeStatusSyncEntity = employeeStatus,
            workPlaces = workPlaces?.map { it.toSyncEntity() }
        )
    }
}